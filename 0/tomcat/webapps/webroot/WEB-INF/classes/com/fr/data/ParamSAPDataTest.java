...
public class ParamSAPDataTest extends AbstractTableData {
    ...
    public ParamSAPDataTest() {
        this.parameters.add(new Parameter("LIFNR"));
        this.parameters.add(new Parameter("NAME1"));
    ...
    public void init() throws JCoException {
        ...
        function.getImportParameterList().setValue("LIFNR", "%" + this.getParameters(null)[0].getValue().toString().toUpperCase().trim() + "%");
        function.getImportParameterList().setValue("NAME1", "%" +  this.getParameters(null)[1].getValue().toString().toUpperCase().trim() + "%");
